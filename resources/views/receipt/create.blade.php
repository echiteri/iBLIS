...
<div>
	<ol class="breadcrumb">
	  ...
	  <li><a href="{!! route('receipt.index') !!}">{!! Lang::choice('messages.receipt',2) !!}</a></li>
		   ...
		   {!! Form::open  (array('url' => 'receipt', 'id' => 'form-receipts', 'method' => 'POST' )) !!}
            <div class="form-group">
               ...
               {!! Form::select('commodity', array(null => '')+ $commodities,
                    old('commodity'), array('class' => 'form-control')) !!}
              </div>
            <div class="form-group">
                {!! Form::label('supplier', Lang::choice('messages.supplier',1)) !!}
                {!! Form::select('supplier', array(null => '')+ $suppliers,
                    old('supplier'), array('class' => 'form-control')) !!}
            </div>
            <div class="form-group">
                {!! Form::label('batch_no', trans('messages.batch-no')) !!}
                {!! Form::text('batch_no', old('quantity'),array('class' => 'form-control', 'rows' => '2')) !!}
            </div>
            <div class="form-group">
                {!! Form::label('quantity', trans('messages.quantity')) !!}
                {!! Form::text('quantity', old('quantity'),array('class' => 'form-control', 'rows' => '2')) !!}
            </div>
            <div class="form-group">
                {!! Form::label('expiry_date', Lang::choice('messages.expiry-date',1)) !!}
                {!! Form::text('expiry_date', old('expiry-date'), array('class' => 'form-control standard-datepicker')) !!}
            </div>
